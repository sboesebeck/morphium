                ...
                public void run() {
                    ...
                    try {
                        for (Map.Entry<Class, List<Object>> es : sorted.entrySet()) {
                            ...
                            if (!dbLst.isEmpty()) {
                                InsertMongoCommand settings = new InsertMongoCommand(morphium.getDriver().getPrimaryConnection(wc))
        ...
        WriterTask r = new WriterTask() {
            ...
            public void run() {
                ...
                try {
                    ...
                    try {
                        StoreMongoCommand settings = new StoreMongoCommand(morphium.getDriver().getPrimaryConnection(wc))
...
//                            if (morphium.getConfig().isAutoIndexAndCappedCreationOnWrite() && !morphium.getDriver().getConnection().exists(morphium.getConfig().getDatabase(), coll)) {
...
//                            }
    ...
    public <T> void convertToCapped(final Class<T> c, String collectionName, final AsyncOperationCallback<T> callback) {
        Runnable r = () -> {
            WriteConcern wc = morphium.getWriteConcernForClass(c);
...
//                            }
                ...
                try {
                    ...
                    UpdateMongoCommand up = new UpdateMongoCommand(morphium.getDriver().getPrimaryConnection(wc))
                            .setDb(getDbName())
        ...
        WriterTask r = new WriterTask() {
            ...
            public void run() {
                ...
                try {
                    ...
                    DeleteMongoCommand settings = new DeleteMongoCommand(morphium.getDriver().getPrimaryConnection(wc))
                            .setColl(collectionName)
                            .setDb(getDbName())
...
//                    if (morphium.getConfig().isAutoIndexAndCappedCreationOnWrite() && !morphium.getDriver().getConnection().exists(getDbName(), coll)) {
...
//                    }
                    ...
                    UpdateMongoCommand settings = new UpdateMongoCommand(morphium.getDriver().getPrimaryConnection(wc))
                            .setColl(coll)
                            .setDb(getDbName())
                            .setWriteConcern(wc.asMap())
        ...
        WriterTask r = new WriterTask() {
            ...
            public void run() {
                ...
                try {
                    if (upsert)
                        checkIndexAndCaps(cls, coll, callback);
//                    if (upsert && morphium.getConfig().isAutoIndexAndCappedCreationOnWrite() && !morphium.getDriver().getConnection().exists(getDbName(), coll)) {
//                        createCappedCollationColl(cls, coll);
...
//                    }
    ...
    private void handleCreationTimeOnUpsert(Class cls, String coll, Map<String, Object> query, Map<String, Object> update, boolean upsert) {
        if (upsert && morphium.getARHelper().isAnnotationPresentInHierarchy(cls, CreationTime.class) && morphium.isAutoValuesEnabledForThread()) {
            ...
            long cnt = 1;
            try {
                CountMongoCommand settings = new CountMongoCommand(morphium.getDriver().getReadConnection(null))
...
//                    }
    ...
    public <T> void pushPull(final boolean push, final Query<T> query, final String field, final Object value, final boolean upsert, final boolean multiple, AsyncOperationCallback<T> callback) {
        WriterTask r = new WriterTask() {
            ...
            public void run() {
                ...
                String fieldName = morphium.getARHelper().getMongoFieldName(cls, field);
                Map<String, Object> set = UtilsMap.of(fieldName, v instanceof Enum ? ((Enum) v).name() : v);
                Map<String, Object> update = UtilsMap.of(push ? "$push" : "$pull", set);
        ...
        if (morphium.getARHelper().isAnnotationPresentInHierarchy(cls, Entity.class)) {
            WriterTask r = new WriterTask() {
                ...
                public void run() {
                    ...
                    try {
                        DropMongoCommand settings = new DropMongoCommand(morphium.getDriver().getPrimaryConnection(null))
                                .setColl(co)
                                .setDb(getDbName());
                        settings.execute();
                    } catch (MorphiumDriverException e) {
    ...
    public <T> void createIndex(final Class<T> cls, final String collection, final IndexDescription idesc, AsyncOperationCallback<T> callback) {
        WriterTask r = new WriterTask() {
            ...
            public void run() {
                ...
                try {
                    CreateIndexesCommand cmd = new CreateIndexesCommand(morphium.getDriver().getPrimaryConnection(null));
    ...
    public abstract class WT<T> implements WriterTask<T> {
        ...
        public void doUpdate(Class cls, T toSet, String coll, String field, Map<String, Object> query, Field f, Map<String, Object> update, WriteConcern wc) {
            ...
            try {
                checkIndexAndCaps(cls, coll, callback);
                UpdateMongoCommand settings = new UpdateMongoCommand(morphium.getDriver().getPrimaryConnection(null))
                        .setColl(coll)
                        .setDb(getDbName())
                        .addUpdate(Doc.of(query), Doc.of(update), null, false, false, null, null, null);
                if (wc != null)
