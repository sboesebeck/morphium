...
#### `morphium-jakarta-data`: derived queries (`findBy...`/`countBy...`/`existsBy...`/`deleteBy...`) map comparison-operator arguments through custom type mappers, same as `@Query`(JDQL) methods and the type-safe `Query<T>` API
`QueryExecutor.buildRawCondition` put a derived method's comparison argument (`args[cond.paramIndex()]`) into the filter map unchanged, reaching `BsonEncoder` without ever passing a custom mapper -- unlike `@Query`(JDQL) methods (`JdqlMethodBridge.applyCondition`, via `field.lt(value)` etc.) and the type-safe API (`MongoFieldImpl.checkValue`, via `marshallIfCustomMapped`), both of which already routed comparison values through the registered custom mappers. With `ObjectMappingSettings#setUseBsonDateForJavaTime(true)`, an entity's `Instant` field is stored as a native BSON Date via `store()`, but a derived method comparing against it (`findByStatusAndExpiresAtLessThan(Status, Instant)`) put the raw `Instant` into `$lt` unmapped, and `BsonEncoder` encodes an unmapped `Instant` in Morphium's legacy sub-document format regardless of the flag -- a different BSON type than the stored value, so the query silently matched nothing. The equivalent `@Query`(JDQL) method for the same field and operator matched correctly, because it never went through this path. `buildRawCondition` now maps every scalar comparison argument (`EQ`, `NE`, `GT`, `GTE`, `LT`, `LTE`, `BETWEEN`) through `Morphium#getMapper()`'s `marshallIfCustomMapped(Object)`, and `IN`/`NIN` map each collection element individually -- `marshallIfCustomMapped` only matches a value whose own runtime class is a registered custom mapper, and a `Collection` itself never is, so mapping the collection as a whole would have left every element unmapped. String-based operators (`LIKE`, `STARTS_WITH`, `CONTAINS`, ...) are unaffected; they already call `toString()` on the argument. Reproduced and pinned against a real MongoDB replica set, not `InMemoryDriver`: the in-memory driver does not normalise values on its write path (#336), so this exact discrepancy is invisible there regardless of whether the mapping gap is fixed.
...
#### InMemoryDriver: six aggregation stages no longer answer wrongly or crash (#395)
`$skip` returned `size - 2*idx` documents and threw `IllegalArgumentException` once `idx` passed half the collection (`subList(idx, size - idx)`); it now drops the first `idx`. `$unwind` broke out of its outer input loop the moment one document had a missing path, so every document after it was lost, and it reassigned the stage spec inside that loop; it now drops only the affected document, supports `preserveNullAndEmptyArrays` and `includeArrayIndex`, treats a scalar operand as a one-element array and resolves dotted paths without mutating the source. `$lookup` joined with a raw `Map.get`, so dotted paths and array-valued join keys found nothing, and `let` without a `pipeline` fell into the advanced branch and joined the whole foreign collection for every input document; both now behave like mongod, and the `let`-without-`pipeline` combination is rejected. `$graphLookup` had `connectFromField` and `connectToField` swapped, ignored `depthField` and `restrictSearchWithMatch`, mutated the input document and returned nothing for `maxDepth: 0`; the traversal now matches `connectToField` against the current value and recurses through `connectFromField`. This is the one visible behaviour change: a pipeline written against the inverted semantics returns different documents now, and the in-memory suite's own graphLookup test had the two fields swapped and was corrected. `$sort` threw `NullPointerException` on a document missing the sort field, ignored dotted paths and collation; it now resolves paths and sorts a missing field as null. `$search` logged an Atlas-only warning and broke, which left the freshly initialised empty result in place and silently dropped every input document; it now fails loudly like the other unsupported stages. Dotted paths in `$lookup`, `$sort` and `$graphLookup` now walk through arrays of subdocuments (`items.sku` collects every element's value, as mongod does), `$sort` on an array-valued field compares its smallest element ascending and its largest descending, `$graphLookup` matches array-valued connect fields element-wise and emits each foreign document once, and `$unwind` with `preserveNullAndEmptyArrays` drops the field of an empty array like mongod.
...
#### PoppyDB: a follower whose sync source steps down mid-copy backs off with one warning instead of an error per attempt - the rest of #364
The replication loop had learnt in 6.3.11 to back off from a source that answers "not primary" (13435/10107/189) instead of reconnecting to it at full speed. Two paths still treated that answer as an ordinary failure. The initial-sync thread: a leader change while a follower is in the middle of its snapshot copy made every further read of the copy answer 13435, and the thread logged an ERROR with a full stack trace per attempt on its regular schedule - and, once the identical error had repeated often enough, escalated to "NODE STUCK IN RECOVERY", for a node that was neither stuck nor in error, merely waiting for an election. And the periodic index sync, which reported the same answer as "failed, will retry in 30000ms". Both now recognise the shape: the sync thread warns once per streak ("no longer the primary ... waits for the leadership change to re-target replication"), retries on the loop's 1s-doubling-to-10s backoff without touching the stuck-node counter, and the index sync says what it is waiting for. 13436 (NotPrimaryOrSecondary, the demoted node already re-syncing from the new leader) counts as the same answer on every path; the old code only knew the first three codes. Deliberately kept: the retry itself - the same host can become primary again through a priority takeover, and the leader-discovery callback does not fire for an unchanged address - and the driver's own re-resolution underneath each read, which is bounded by `retriesOnNetworkError` at about one attempt per heartbeat and ends the moment the leadership change replaces the manager. Measured on `StepdownReplicationTest` and `PrimaryReadStepdownRetryTest`: the manager itself no longer produces a single 13435 line in the stepdown window; what remains are the bounded per-read retries of the driver. `SyncSourceStepdownDuringInitialSyncTest` parks a follower's copy with the #323 seam, freezes the other two nodes so the set stays leaderless, steps the source down and releases the copy: one WARN and no ERROR across three further attempts (three ERRORs with stack traces before). Two things from the review of that change: the replication loop's own reaction to the stepped-down answer - close the connection and null the primary client - could hit the copy between two reads, and the copy's NullPointerException or closed-connection error carries no code, so it fell into the ERROR path after all; the sync thread now treats "the loop dropped the connection" as the same condition (second scenario of the test: the source is dropped mid-copy, one WARN, the copy completes after the reconnect). And the message-based recognition of the codes is anchored on the driver's `Error: CODE -` shape, since a random test port or a document count with the same digits must not read as a stepdown.

#### InMemoryDriver: the aggregation pipeline compares longs past 2^53 exactly - $bucket boundaries, window sorting and ranking, $sortArray/$maxN/$minN, set membership (#381)
#379 made the matcher and the index comparator exact past 2^53 and assumed the pipeline follows; it did not. Three comparators still routed every Number through `doubleValue()`: `InMemAggregator.compareValues` behind the `$bucket` boundary test and the `$bucketAuto` ordering, `InMemAggregator.compareSortValues` behind `$setWindowFields` (sortBy, `$rank`/`$denseRank`, the `$first`/`$last`/`$min`/`$max` windows), and `Expr.compareValues` behind `$sortArray`, `$maxN` and `$minN` - plus `Expr`'s set membership (`$setUnion`, `$setDifference`, `$setIsSubset`, ...), which merged 2^53 and 2^53+1 into one element. A pipeline could therefore `$match` a snowflake id exactly and then file it in the wrong bucket: 2^53 landed in [2^53+1, 2^53+2), two documents at 2^53 and 2^53+1 both got rank 1, `$maxN` over them answered 2^53. All of them now delegate to `QueryHelper.compareNumbers`, the #379 cascade (integral vs integral via `Long.compare`, integral vs double by exact value, double vs double as before), which is public for that. The pipeline `$sort` stage was never affected - it compares through `Comparable` - and `$graphLookup` keeps its double comparison, since it normalises its connect values to double for hashing anyway. `$densify` keeps its double comparison as well; its generated points are computed in double anyway. Review follow-up: the output accumulators of `$bucket`/`$bucketAuto` have their own `$min`/`$max`, apart from `$group`'s, and folded through double too - the boundary test was exact, the bucket's own `mx` was not, and a long field came back as a Double; they now keep the value and compare exactly. Values within +-2^53 are unchanged.

#### PoppyDB: a partial restore that hits every node identically no longer leaves the set without a primary (#391)
The #306 candidacy guard is right for one broken node: a node that could not restore all of its dump files holds back candidacy until an initial sync from a primary has given it a complete copy, so it can never push its incomplete state onto intact peers. But the guard's only release was that sync, and a sync needs a primary. When the SAME dump file failed on EVERY node - #390's regex token, a dump written by a newer version, a corrupted file replicated to all nodes - all three nodes held back, nobody could become primary, and nothing ever released the guard: on the test runner on 2026-09-21 the set stood without a primary for 25 minutes, until the file was moved out of every dump directory by hand and every node restarted. On the acceptance message bus the same failure would be a total outage that only hand work ends. Three changes. (1) A held-back node now asks its peers for their restore finding on every election timeout, over its own read-only admin command `poppyRestoreStatus` (the heartbeat cannot carry it - only a leader sends one, and the finding is needed precisely while there is none - and the PreVote is not sent by a held-back node at all); a peer answers with whether its data is complete, which dump files failed, its priority and whether it knows a leader. When EVERY peer - not a majority - has answered the current round with incomplete data and exactly the same failed-file set, none knows a leader, and this node has the highest priority among them (ties by the lower address), it lifts its guard with a WARN naming the files ("no peer holds a complete copy of [...]") and campaigns; the others keep their guard and take the new primary's state through the existing sync path. A peer with a complete copy or a different loss, a peer that does not answer (down, or an older version that does not know the command) and a node whose data is incomplete for another reason than a failed restore (store emptied for a sync) all keep the guard in place - fail closed, nothing changes for the one-broken-node case. (2) An operator command `poppyAcceptPartialRestore` (admin database) lifts the guard on that node at once, answering `ok:1` with `lifted` and the failed file names - for mixed-version sets where the exchange cannot complete, or to pick a specific node; refused (`lifted:false`) when the store was emptied for a sync. (3) The `holding back candidacy` WARN names the failed dump files instead of sending the operator back to the startup log, and repeats at most once a minute instead of on every election timeout (85 lines in the 100 seconds of the reproducing test before). `PartialRestoreClusterWideTest` restarts a three-node set with the same unreadable file in every dump directory and requires a primary within 30 s - the highest-priority node - and both other nodes synced; before the fix: "got 0 primaries". Its second scenario keeps one node's copy intact and requires that node to win as before.

#### Driver: a primary read answered "not primary" during a stepdown or priority takeover is retried on the new primary - as a write already was (#393)
`ReadMongoCommand` retried only network exceptions. When the node behind a primary connection answered with the not-primary family instead - mongod's 13435 `NotPrimaryNoSecondaryOk`, 10107 `NotWritablePrimary`, 189 `PrimarySteppedDown`, 11602 `InterruptedDueToReplStateChange`, 91/11600 at shutdown; PoppyDB's 13435 and 10107 from the same gate, plus 13436 `NotPrimaryOrSecondary` while a demoted node re-syncs - the exception went straight to the caller, although the driver's heartbeat would have known the new primary a few hundred milliseconds later. `WriteMongoCommand` had handled exactly this since its `isStepDownError`. Seen on the test runner in 3 of the last 4 `mongodb_rs` runs (2026-09-21/22), always as the first method of the class that runs right after `DriverFailoverProxyTest`: that test leaves mongo1 (priority 100) stepped down with mongo2 as interim primary, mongod schedules mongo1's priority takeover 10 s later, and the next class connected inside that window - its `dropCollection` polled `exists()` (`listCollections` on the primary) at the very moment mongo2 stepped down: `Error: 13435 - not primary and secondaryOk=false`. On a PoppyDB bus every rolling restart ends in the same takeover, so every client with a primary read in flight at that instant got one exception. The classification now lives in `StepDownErrors`, shared by both paths (13436 is new to the write path as well - a recovering node has refused the write before touching anything). A read that targets the primary (`ReadPreference.primary`/`primaryPreferred`, or a connection from `getPrimaryConnection`) and gets one of these answers is retried: the driver is asked to refresh its primary from the very node that rejected the read (`MorphiumDriver.refreshPrimaryAfterStepDown`: `PooledDriver` sends it a hello and handles the reply like a heartbeat's, so a stepped-down node that already names the winner switches the driver over at once, one that does not clears the primary and the retry waits for the heartbeat to find it), the connection is given back, and after `sleepBetweenErrorRetries` (at least 50 ms) the read goes out again on the re-resolved primary, at most `retriesOnNetworkError` times, one WARN per retry. A read with `secondaryPreferred`/`secondary`/`nearest` keeps the error - there it is the caller's read preference meeting a node that does not serve it, not a failover - and so does a read inside a transaction, which the transaction layer restarts as a whole. Only a cursor's first reply is covered: a getMore of a partially consumed cursor cannot move to another node, and re-running the query would repeat documents the caller has seen. Two things the PoppyDB replica-set test of this change turned up on the way: a stepped-down PoppyDB node still named *itself* as `primary` in its hello - #392 had cleared the ElectionManager's leader, but `MongoCommandHandler.getHelloResult` fell back to the cached `primaryHost`, so the driver's re-resolution ("re-resolved from its own hello") stayed on the demoted node for all five retries; in election mode the hello now names the ElectionManager's leader or nobody. And the driver no longer believes a hello that says `isWritablePrimary: false` together with `primary: <the node itself>`, from any server: a self-contradictory answer names nobody, and the heartbeat finds the winner. Until the winner is elected the *followers* still name the old primary in their hellos and the driver takes them at their word, so a retry can land on the demoted node once more per heartbeat interval; the budget therefore covers about `retriesOnNetworkError` heartbeat intervals of election (10 s with the defaults - in the PoppyDB test the fourth attempt, 2.9 s after the stepdown, reached the new leader). Not changed: `DriverFailoverProxyTest` still hands the shared set back mid-takeover; a passive wait-until-settled there is a separate follow-up.

#### PoppyDB: a stepped-down node no longer advertises itself as primary until the next leader's heartbeat (#392)
`ElectionManager.stepDown` went to FOLLOWER via `becomeFollower(term, null)`, and a null leader keeps the previous one - after a stepdown that is the node itself. Until the next leader's first heartbeat arrived (5 s on the test runner), `hello` on the stepped-down node answered `isWritablePrimary: false` together with `primary: <itself>`; the driver's recovery path took that at face value, retried the same node, met its recovering gate (13436, the full sync that follows a leader change) and looped: "Primary failover? 17018 -> 17018 (re-resolved from 17018's own hello)" seven times in 5 ms, then `Error: 13436 - node is recovering` in `AdvancedMessagingTests`, which had connected 3 s after a priority-takeover yield. Every stepdown and every takeover yield opened that window - on the acceptance bus every rolling restart. The stepdown now clears the known leader, as mongod reports no primary in that window; the driver waits for the real one instead.

#### InMemoryDriver/PoppyDB: a regex value in any document made that database's dump unrestorable - and took the whole replica set down with it (#390)
The dump writer (`writeDumpJson`) knew Date, UUID, ObjectId and byte[] and wrote everything else as a bare `toString()` token. A BSON regex is decoded into a `java.util.regex.Pattern` and stored as such, so a document with a regex field was dumped as `"pattern_value" : a*` - not JSON. On restore the parser failed the whole database file, and because every node of a replica set holds the same data, every node failed the same file: partial restore on all three, the #306 candidacy guard held all three back, and the set had no primary until the file was removed by hand. Seen on the test runner on 2026-09-21 after a rolling restart (the regex field of the wire-type test in `morphium_tests_2`), 25 minutes without a primary and every test of the phase failing with "No primary node found". Regex and `MongoTimestamp` values are now written as `class_name` markers and restored to their type (a regex keeps its flags); every other value the writer has no form for is written as a JSON string with a one-time WARN per type, so a database can lose a value's type but never its dump. The PoppyDB dump path also no longer runs the store through the entity mapper, which rejected a timestamp or MinKey/MaxKey value outright ("Cannot write object to db that is neither entity, embedded nor serializable") - no dump at all for that database, every interval. Dumps written by earlier versions with a bare regex token remain unreadable; recover by moving the broken file out of the dump directory before the restart.
...
#### PoppyDB: index builds and drops replicate as change stream events - an index created shortly before a leader change is no longer lost (#386)
Index replication (#258) was periodic only: a `createIndexes` on the primary ran locally, and followers picked it up by diffing `listIndexes` every 30s. The change stream carried no index DDL. A leader change inside that window - the priority takeover fires after 30s of leader stability, so it is built in whenever the preferred node does not win the first election - promoted a follower that had never received the index, and the remaining followers, the old primary among them, then dropped it while aligning to the new primary (`applyIndexDiff`: "dropped stale index"). Nothing recreated it: a lost TTL index stops data from expiring, a lost unique index drops the constraint, and neither produces an error anywhere. Seen twice in one day on `FullClusterRestartTest`, which fails whenever the lower-priority node wins the first election. The `InMemoryDriver` now emits MongoDB's expanded change stream events for index DDL - `createIndexes` and `dropIndexes`, with the full spec in `operationDescription.indexes` - through the same path as a write, so they get a sequence token, sit in the replay buffer and are dispatched in order with the surrounding data. As on mongod they only reach a watch that asked for them with `showExpandedEvents: true`; messaging, cache synchronisation and application change streams see nothing new. PoppyDB's replication asks for them and applies both in order with the data, idempotently, so a follower holds an index before it can be elected and a new primary has everything its predecessor committed. The periodic diff stays as the safety net for changes whose events fell out of the replay window, and its rule that the primary is always right is unchanged - with the event in place, an index the primary lacks is either one it dropped or one whose build never reached a majority before the failover, the w:1 write MongoDB rolls back in the same situation.
...
#### PoppyDB: the election timeout orders candidates by priority instead of merely biasing them (#387)
`ElectionConfig.randomElectionTimeout()` was `random(min..max) + range * (1 - priority/100)`: the random span was as wide as the whole priority delay, so any two priorities less than 100 apart drew overlapping windows - 100 vs 90 gave 2.0-4.0s against 2.2-4.2s - and the lower-priority node campaigned first in a sizeable share of elections (about one in five for 50 vs 10, far more for 100 vs 90). On the acceptance bus (100/50/25) node3 won against node2 whenever node1 was away, to be corrected by a priority takeover 30s later: two leader changes instead of one, each costing every client its resume tokens. The range `[min, max)` is now divided into `MAX_PRIORITY / electionPriorityStep` slots (step 10 by default, `morphiumserver.electionPriorityStep`); a node's slot is `(100 - priority) / step`, and it randomizes over the first half of its slot, the other half being the guaranteed gap. Priorities at least one step apart therefore never overlap - every timeout of the higher one is shorter than every timeout of the lower one - while equal priorities keep randomizing against each other. The formula scales with the configured min/max (2000..4000ms: 200ms slots, 100ms jitter; the test runner's 5000..10000ms: 500ms slots). Two visible timing changes: the highest priority times out at the minimum (2.0-2.1s instead of anywhere in 2.0-4.0s), and the lowest stays below the maximum instead of a full range above it (3.8-3.9s instead of 3.8-5.8s). `ElectionConfigTest` pins the ordering over thousands of draws; `PriorityOrderedElectionTest` steps a 100/50/40 set down twenty times and requires the highest-priority survivor to win every round (6 of 20 rounds wrong with the old formula).

#### PoppyDB: the priority takeover says why it withholds, learns a peer's position from its heartbeats, and judges "caught up" by a freshness window (#388)
With priorities 100/75/50, the preferred node electable and unfrozen for over a minute, the leader never yielded on the test runner (three scenarios, 0 `yielding leadership`) - and every reason for that was logged at DEBUG, so the field logs could not say which. Locally the picture was different: in the fifteen stepdowns of the report every leader term but one was ended by the test's own next stepdown before the 30s stability window had passed, and the one term that reached it yielded at exactly 30.0s. A reproduction with three nodes and a write load found the check withholding for a *legitimate* reason - at ~25.000 inserts/s the demoted node's full resync (forced by its non-replicated collections) never drained its backlog and it was genuinely 400.000 events behind - and yielding at the first check under a few hundred inserts/s. What the runner's leader saw remains unproven; three things change so the next occurrence is answerable and two known gaps are closed. (1) The reason a takeover is withheld is recorded per peer (`takeoverWithheld` in the election stats) and logged at INFO when it first appears or changes kind, once a minute otherwise. (2) The leader learned a follower's position only from `replSetProgress` acks, kept in a `ReplicationCoordinator` that is recreated on every leadership change and only fed by a follower whose position moved past what it last reported - to whichever primary that was - so a fully caught-up peer could stay unknown (`-1`, "no replication progress reported") on an idle set; the heartbeat response now also carries the follower's position (`appliedSequence`, in the leader's sequence space, from `ReplicationManager.getLastAppliedSequence()`) and the leader uses the better of both channels. As with `electable` (#385) the field is optional: a node from before this release omits it and the ack bookkeeping is used as before. (3) "Caught up" was `lag == 0` against the leader's sequence *of this very instant*, a condition that depends on where the 2s sample falls relative to the last write and the follower's 50ms ack; it is now a freshness window as in mongod - a peer counts as caught up once it has acknowledged everything the leader had at the *previous* takeover check, one check interval earlier - so a follower that keeps pace qualifies regardless of sampling luck, and one that has stopped applying still does not. Also: an AppendEntries arriving while the manager shuts down no longer runs `becomeFollower` into a `RejectedExecutionException` - callbacks are dispatched through a guard that skips a stopped scheduler.
