...
#### Messaging: liveness-gated fallback poll — zero polling while streams are healthy
The change-stream watch loop receives a server reply at least every `maxTimeMS` (an empty batch when there are no events); that heartbeat is now stamped on the `WatchCommand` and exposed as `ChangeStreamMonitor.isStreamLive()`. Both messaging implementations use it to gate their safety-net poll: a topic whose streams are provably alive and in sync is not polled at all (the resume token advances with every batch, so events cannot silently go missing), and a stream falling silent triggers an *immediate* poll — faster than any timer — followed by one per `messagingFallbackPollInterval` while it stays suspect. `SingleCollectionMessaging` (which had its own counter-based gate, effectively ~25s) gets the same treatment, plus the catch-up poll on every watch (re-)establishment for its message and lock monitors — including the one recreated by its stall watchdog. New diagnostics: `MultiCollectionMessaging.topicStreamsLive(topic)` and `SingleCollectionMessaging.changeStreamsLive()`.
