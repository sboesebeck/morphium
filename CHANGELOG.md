...
#### Driver: `connectionSettings().setRetriesOnNetworkError()` and `setSleepBetweenNetworkErrorRetries()` now reach the driver
Morphium copied every other connection setting onto the driver it builds, but not these two, so every retry budget in the driver - network errors, the stepdown retries of #393 - ran on the driver's own defaults (5 retries, 100 ms) whatever the config said. Noticed in PoppyDB's ReplicationManager, which configures 3 and 500 ms and logged "retry 5/5". Both are now applied. So that nothing changes for a client that never set them, the config defaults are now the driver's own, 5 and 100 ms; the documented 1 and 1000 ms were never in effect. A client that did set them gets, for the first time, what it asked for. The InMemoryDriver keeps the values as well instead of answering constants.
