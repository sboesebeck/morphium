...
public class QueryBuilderTest extends MultiDriverTestBase {
    ...
    public void testQuery(Morphium morphium) {
        ...
        assertTrue((str.trim().equals("{ \"$or\" :  [ { \"counter\" : { \"$lte\" : 15 }  } , { \"counter\" : { \"$gte\" : 10 }  } , { \"$and\" :  [ { \"counter\" : { \"$lt\" : 15 }  } , { \"counter\" : { \"$gt\" : 10 }  } , { \"str_value\" : \"hallo\" } , { \"str_value\" : { \"$ne\" : \"test\" }  } ] } ] }")), "Query-Object wrong");
        q = q.q();
        ...
        assertNotNull(str, "ToString is NULL?!?!?");
        System.out.println("Query: " + str);
        assertTrue((str.trim().equals("{ \"counter\" : { \"$mod\" :  [ 10, 5] }  }")), "Query wrong");
        q = q.q(); //new query
    ...
    public void testComplexAndOr(Morphium morphium) {
        ...
        log.info("Query: " + s);
        assertTrue((s.trim().equals("{ \"$and\" :  [ { \"counter\" : { \"$lt\" : 100 }  } , { \"$or\" :  [ { \"counter\" : 50 } , { \"counter\" : 101 } ] } ] }")));
    }
    ...
    public void testToString(Morphium morphium) {
        ...
        assertTrue((Utils.toJsonString(q.toQueryObject()).trim().equals("{ \"long_list\" : { \"$size\" : 10 }  }")));
    }
    ...
    public void testWhere(Morphium morphium) {
        ...
        assertTrue((q.toQueryObject().get("$where").equals("this.value=5")));
    }
    ...
    public void testF(Morphium morphium) {
        ...
        assertTrue((f.getFieldString().equals("_id")));
    ...
    public void testOverrideDB(Morphium morphium) {
        Query<UncachedObject> q = morphium.createQueryFor(UncachedObject.class);
        assertTrue((q.getDB().equals(morphium.getConfig().connectionSettings().getDatabase())));
        q.overrideDB("testDB");
        assertTrue((q.getDB().equals("testDB")));
    }
    ...
    public void testOr(Morphium morphium) {
        ...
        ;
        assertTrue((((java.util.List<?>) qo.get("$or")).size() == 2));
    ...
    public void testNor(Morphium morphium) {
        ...
        ;
        assertTrue((((java.util.List<?>) qo.get("$nor")).size() == 2));
    ...
    public void testQ(Morphium morphium) {
        ...
        q.limit(12);
        q.sort("strValue");
        assertTrue((q.q().getSort() == null || q.q().getSort().isEmpty()));
        assertTrue((q.q().getWhere() == null));
        assertTrue((q.q().toQueryObject().size() == 0));
        assertTrue((q.q().getLimit() == 0));
    }
}
