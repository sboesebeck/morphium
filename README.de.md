...
```bash
curl -O https://repo1.maven.org/maven2/de/caluga/poppydb/6.3.11/poppydb-6.3.11-cli.jar
...
java -jar poppydb-6.3.11-cli.jar --port 27017 --no-config
```
...
```bash
java -jar poppydb-6.3.11-cli.jar --port 27017 --dump-dir ./data --dump-interval 300
```
...
```bash
java -jar poppydb-6.3.11-cli.jar -p 17017 --rs-name myrs \
...
- Live-Monitoring & Lasttests: [Morpheus](https://github.com/sboesebeck/morpheus) — Terminal-UI/CLI, mit dem man Nachrichten, Topics und Node-Health live beobachten kann, ohne eigenen Code
...
### Optionale Integrationsmodule
`morphium-jakarta-data` implementiert [Jakarta Data 1.0](https://jakarta.ee/specifications/data/1.0/) auf Basis von Morphiums Query-Engine — `@Repository`-Interfaces mit Query-Ableitung aus Methodennamen, JDQL über `@Query` (inklusive `GROUP BY`/`HAVING`, übersetzt in eine Aggregation-Pipeline), Offset- sowie Cursor-/Keyset-Pagination. Zwei Framework-Module setzen darauf auf: `quarkus-morphium` für die CDI-Integration (Config-Mapping, `@MorphiumTransactional`, Health-Checks, Dev Services, Dev UI, GraalVM-Native-Image-Support, Repository-Generierung zur Build-Zeit per Gizmo), und `spring-boot-morphium` für Spring Boot (Auto-Konfiguration, typsichere `@ConfigurationProperties`, `@MorphiumTransactional`, ein Actuator-Health-Indicator) — eine Spring-Boot-App auf Morphium, kein Ersatz für Spring Data MongoDB. Alle drei sind optional — der Core hängt von keinem ab, und `-DskipExtensions` erzeugt weiterhin einen reinen Core-Build. Siehe [Jakarta Data](docs/jakarta-data.md), [Quarkus-Extension](docs/quarkus-extension.md) und [Spring Boot Starter](docs/spring-boot.md).
