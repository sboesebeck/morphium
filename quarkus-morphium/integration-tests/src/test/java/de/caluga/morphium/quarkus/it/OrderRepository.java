...
public interface OrderRepository extends BasicRepository<OrderEntity, String> {
    ...
    // -- Regression: dynamic Sort parameter on a derived deleteBy*/countBy*/existsBy* method
    // (previously fell through to the FIND branch of QueryMethodBridge#executeQuery, so
    // deleteByStatus(Sort) deleted nothing while still returning a "successful" count, and
    // countByStatus(Sort)/existsByStatus(Sort) threw a ClassCastException because a List came
    // back where the generated bytecode expected a Long/boolean) --

    long deleteByStatus(String status, Sort<OrderEntity> sort);

    long countByStatus(String status, Sort<OrderEntity> sort);

    boolean existsByStatus(String status, Sort<OrderEntity> sort);
