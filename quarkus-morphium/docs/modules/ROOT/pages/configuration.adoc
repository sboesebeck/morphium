...
| `60`
| Time-to-live in seconds for the migration lock. Must be greater than `0`. The lock is renewed (heartbeat) after every executed migration, so this only needs to exceed the time a single change unit's `execute()` can take, not the whole migration run. Computed from each instance's local clock, not the MongoDB server's -- keep replica clocks synchronized (NTP/chrony) and set this generously above the expected clock drift between instances, not just above expected migration runtime.
