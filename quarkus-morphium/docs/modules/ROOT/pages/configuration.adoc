...
| –
| Store `Instant`, `LocalDate`, `LocalTime` and `LocalDateTime` as BSON `ISODate`. Left unset, the extension keeps its previous per-type behaviour. Setting it to `false` is not "keep things as they are": `LocalDateTime` defaults to `ISODate` today via the deprecated property below, so `false` here flips it to the legacy Map format.
